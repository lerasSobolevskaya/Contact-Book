...
public class AbstractDaoMySQL {
	...
	protected static final String SELECT_ALL_USERS = "SELECT user.user_id, user.name, user.surname, user.role, user.login, user.password, contact.contact_id, contact.phone, contact.email, contact.user_id FROM users user LEFT OUTER JOIN contacts contact ON user.user_id=contact.user_id";
	protected static final String SELECT_USER_BY_ID = "SELECT user.user_id, user.name,user.surname,user.role,user.login,user.password, contact.contact_id, contact.phone, contact.email, contact.user_id FROM users user LEFT OUTER JOIN contacts contact ON contact.user_id=contact.user_id WHERE user.user_id=?";
	protected static final String CREATE_USER = "INSERT INTO users (name, surname, role, login, password) VALUES (?,?,?,?,?)";
	protected static final String UPDATE_USER = "UPDATE users SET name=?, surname=? WHERE user_id=?";
	protected static final String DELETE_USER = "DELETE FROM users WHERE user_id=?";
	protected static final String SELECT_LOGIN_PASS = "SELECT user.user_id, user.name, user.surname, user.role, user.login, user.password, contact.email, contact.phone, contact.user_id FROM users user LEFT OUTER JOIN contacts contact ON user.user_id=contact.user_id WHERE login=? AND password=?";
	protected static final String SELECT_PASS = "SELECT name, surname, role, login, password FROM users WHERE login=? AND password=?";

	protected static final String UPDATE = "UPDATE users SET name=?, surname=? WHERE user_id=?";
	
	protected static final String SELECT_ALL_USERS_TEST = "SELECT user.user_id, user.name, user.surname, user.role, user.login, user.password ,contact.contact_id,contact.email, contact.phone,contact.user_id FROM users user INNER JOIN contacts contact ON contact.user_id=user.user_id";

	protected static final String SELECT_ALL_CONTACTS_TEST = "SELECT contact.contact_id, contact.email, contact.phone, contact.user_id, user.user_id, user.name, user.surname, user.role, user.login, user.password FROM contacts contact LEFT OUTER JOIN users user ON contact.user_id=user.user_id";

	protected static final String SELECT_CONTACT_BY_ID = "SELECT contact.contact_id, contact.phone, contact.email,contact.user_id, user.user_id, user.name, user.surname, user.role, user.login,user.password FROM contacts contact LEFT OUTER JOIN users user ON contact.user_id=user.user_id \r\n" + "WHERE contact.contact_id = ?";
